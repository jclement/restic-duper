...
- `${VAR}` references anywhere in the file are expanded from the environment
  at load time (bare `$VAR` is left alone, so passwords containing `$` are
  safe). Referencing an unset variable is an error.
...
This works directly with services that accept arbitrary JSON POSTs (ntfy,
Discord/Slack via a small relay, healthchecks.io ping URLs, etc.).
...
- restic-duper never prints passwords; they are handed to restic only through
  its documented environment variables.
